#!/usr/bin/env python3
"""
KIE Model Evaluation Script (OpenAI API)

This script evaluates different OCR/KIE models on JSONL datasets using
OpenAI-compatible API for model inference.
Supports classification statistics by 增值税发票, 机动车发票, and 身份证.
"""
...
def extract_category(image_url: str) -> str:
    """Extract category from image_url: 增值税发票, 机动车发票, or 身份证."""
    url = image_url.replace('\\', '/')

    if '增值税普通发票' in url or '增值税' in url:
        return '增值税发票'
    elif '机动车发票' in url or '机动车' in url:
        return '机动车发票'
    elif 'traindata' in url or 'train_data' in url or '身份证' in url:
        return '身份证'
...
class StatisticsCollector:
    ...
    def record_sample(
        ...
        category = extract_category(image_url)

        self.category_stats[category]['total'] += 1

        if processed:
            self.category_stats[category]['processed'] += 1
            self.category_stats[category]['accuracy_sum'] += accuracy
            self.category_stats[category]['total_fields'] += total_fields
            self.category_stats[category]['correct_fields'] += correct_fields
    ...
    def get_category_summary(self) -> Dict[str, Any]:
        ...
        for category, stats in sorted(self.category_stats.items()):
            ...
            field_accuracy = (
                stats['correct_fields'] / stats['total_fields']
                if stats['total_fields'] > 0 else 0.0
            )
            summary[category] = {
                'total': stats['total'],
...
def evaluate_dataset(
    ...
    for idx, item in enumerate(data):
        ...
        try:
            ...
            sample_result = {
                ...
                'category': extract_category(image_path),
                'prompt': prompt,
            ...
            category = extract_category(image_path)
            msg = f"Sample {idx} [{category}]: Accuracy = {comparison['accuracy']:.2%}"
            print(msg)
            log_entries.append(msg)
    ...
    evaluation_results['category_stats'] = stats.get_category_summary()
...
def print_summary(results: Dict[str, Any]):
    ...
    print("-" * 70)
    print(f"{'Category':<15} {'Total':>8} {'Processed':>10} {'Failed':>8} {'Accuracy':>10} {'Field Acc':>10}")
    print("-" * 70)
    for category, stats in results.get('category_stats', {}).items():
        print(f"{category:<15} {stats['total']:>8} {stats['processed']:>10} {stats['failed']:>8} {stats['accuracy']:>9.2%} {stats['field_accuracy']:>9.2%}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Evaluate KIE models using OpenAI API')
