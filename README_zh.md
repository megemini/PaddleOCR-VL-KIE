...
> 💥💥💥 PaddleOCR-VL-KIE 是 [PaddleOCR-VL-XFUND](https://aistudio.baidu.com/projectdetail/10253559) **Pro** **Max** 版本模型，使用 OCR-KIE 数据集
>
> **Pro** 
>
...
本文提出一种针对文件、文本类的基于字符像素密度的图片缩放算法，算法基于以下假设：

> **光学字符识别任务中，每个字符所占用的像素数只要大于某一个阈值，就不会对识别效果再产生实质影响。**

或者说，每个字符只需不少于一定像素即可保证识别，过多像素收益有限。

这里设计了一个实验，用于检测模型的 `视力`，下面是一张 `视力表`，从上到下、从大到小随机显示 `由 日 月 田 正 口` 几个字：

![](https://ai-studio-static-online.cdn.bcebos.com/836834bd742e4376a7a4353a9c3148aa678cce7be06348158c3d51b8a205e766?v=1)

测试模型在哪一种文字大小出现识别率下降的情况，每一行的文字平均单个字符所占像素数量如下：
...
结果表明，在字符密度大约 `50 pix/char` 出现识别率下滑，据此，可以作为后续应用的基础。
...
4. **PaddleOCR-VL-KIE-Q8_0 与 FP16 版本表现几乎一致**（75.81% vs 75.99%），量化对精度影响极小，速度快约 30%
